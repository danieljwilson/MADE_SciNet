...
def values_for_simulation_addm(expdata_file_name, sims_per_val):
    ...
    values_array = np.repeat(values, sims_per_val, axis=0) # duplicate values * simsPerVal
    
    # PRINT SUMMARY
    print("You will run {0} simulations for each of {1} value combinations for a total of {2} simulations.".format(sims_per_val, len(values), values_array.shape[0]))
...
def total_sim_count(parameter_combos, values_array):
    ...
    print("Sim # (per loop) = ", len(values_array))
    print("Total sims = ", len(parameter_combos)*len(values_array))
...
def save_sim_subjects_csv(dfOut, loop_num):
    """
    """
    now = dt.datetime.now()
    time = now.strftime("%Y-%m-%d-%H-%M")
...
def create_dwell_array(num_sims, fixations, exp_data=None, data):
    """
    ...
    first_fix_dist = fixations['fix_time'][fixations['fix_num']==1]
    mid_fix_dist = fixations['fix_time'][(fixations['fix_num']>1) & (fixations['rev_fix_num']>1)]
    ...
    if data == 'test':
        while min_time < 10000:   # 10,000 = maxRT
            ...
            # overwrite first columns with actual fixation values
            row_start = 0
            row_end = 1000
            column = 0
...
def simul_addm(parameter_combos, values_array_addm, dwell_array, num_sims, loop_num,
    ...
    scaling = parameter_combos[loop_num,0]                  # drift scaling on loop
    upper = parameter_combos[loop_num,1]  
    theta = parameter_combos[loop_num,2]                    # boundary on loop
    within_bounds = np.ones((num_sims), dtype=bool)         # initialize location vector for each drift
    ddms = np.zeros(num_sims) + startVar * (np.random.rand(num_sims) - .5)  # -.5 to mean center (starting dist. uniform)
    resp = np.nan * np.ones(num_sims)                       # initialize choices
    rt = np.nan * np.ones(num_sims)  
    ...
    # Left and Right drift (implies which item is being attended to)
...
def rtDistFunc(nonDec, values_array, path_to_save):
    ...
    for param_combo in tqdm(all_sims):                 # loop through the simulations for each parameter combo that was simulated
        ...
        value_pairs = np.unique(values_array, axis=0)
